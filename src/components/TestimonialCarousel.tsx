...
const TestimonialCarousel = ({ testimonials }: TestimonialCarouselProps) => {
  ...
  return (
    <div className="animate-fade-in mt-8 w-full max-w-3xl mx-auto px-0">
      <h3 className="text-base mb-6 text-center sm:text-base text-[calc(0.875rem+1px)]">Testimonios de Estudiantes</h3>
                ...
                <DialogTrigger asChild onClick={() => handleThumbnailClick(testimonial.videoUrl, testimonial.name)}>
                  ...
                  >
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-4 rounded-b-xl">
                      <h4 className={`text-white font-medium ${isMobile ? 'text-[calc(0.875rem+5px)]' : 'text-base sm:text-base'}`}>{testimonial.name}</h4>
                      <p className={`text-white/80 ${isMobile ? 'text-[calc(0.75rem+5px)]' : 'text-sm sm:text-sm'}`}>{testimonial.role}</p>
                    </div>
                  </div>
                </DialogTrigger>
